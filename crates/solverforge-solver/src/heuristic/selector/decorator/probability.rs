...
mod tests {
    ...
    fn same_seed_produces_same_selection() {
        ...
        let moves1: Vec<_> = prob1.iter_moves(&director).filter_map(|m| m.to_value().copied()).collect();
        let moves2: Vec<_> = prob2.iter_moves(&director).filter_map(|m| m.to_value().copied()).collect();
