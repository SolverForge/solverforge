...
mod tests {
    ...
    #[test]
    fn test_acceptor_from_config() {
        let config = AcceptorConfig::HillClimbing;
        let acceptor = acceptor_from_config::<DummySolution>(&config);
        assert!(matches!(acceptor, AcceptorImpl::HillClimbing(_)));
    }

    #[test]
    fn test_termination_from_config_time() {
        let config = TerminationConfig {
            seconds_spent_limit: Some(60),
            ..Default::default()
        };
        let termination = termination_from_config::<DummySolution>(&config);
        assert!(matches!(termination, TerminationImpl::Time(_)));
    }

    #[test]
    fn test_termination_from_config_step_count() {
        let config = TerminationConfig {
            step_count_limit: Some(1000),
            ..Default::default()
        };
        let termination = termination_from_config::<DummySolution>(&config);
        assert!(matches!(termination, TerminationImpl::StepCount(_)));
    }
}
