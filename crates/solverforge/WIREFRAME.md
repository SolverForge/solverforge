...
Extractor ergonomics: `for_each` accepts `CollectionExtract<S, Item = A>`. Raw keyed join targets accept slice closures, `vec(|s| &s.field)`, and macro-generated solution source methods. Passing a right-hand `UniConstraintStream` remains the preferred form when the source needs its own `.filter(...)`.
