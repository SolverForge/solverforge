use super::{ConstructionListElementId, ConstructionSlotId};

#[derive(Debug, Default, Clone)]
pub(crate) struct ConstructionFrontier {
    standard_completed_at_revision: Vec<Vec<u64>>,
...
impl ConstructionFrontier {
    ...
    pub(crate) fn reset(&mut self) {
        for slots in &mut self.standard_completed_at_revision {
            slots.clear();
        }
    ...
    pub(crate) fn is_standard_completed(
        &self,
    ...
    pub(crate) fn mark_standard_completed(
        &mut self,
        slot_id: ConstructionSlotId,
        solution_revision: u64,
    ) {
        let slot = self.ensure_standard_slot(slot_id);
    ...
    fn ensure_standard_slot(&mut self, slot_id: ConstructionSlotId) -> &mut u64 {
        ...
        let slots = &mut self.standard_completed_at_revision[slot_id.binding_index()];
