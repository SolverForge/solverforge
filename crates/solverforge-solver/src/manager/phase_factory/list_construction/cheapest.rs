//! Public facade for canonical source-indexed cheapest insertion.
...
use super::ScoredListConstructionAccess;
...
pub(crate) use kernel::run_cheapest;
#[cfg(test)]
pub(crate) use kernel::{CheapestInsertionObserver, CheapestInsertionTrial};
pub(crate) use live::PhaseCheapestInsertionObserver;

/// List construction phase using the canonical cheapest-insertion kernel.
///
/// The source key is mandatory.  It is the one identity used to bind
/// declarations, resolve assigned values, retain trace positions, and map
/// precedence successors; the phase never falls back to payload equality or
/// hashing.
...
where
    ...
    E: Copy + Send + Sync + 'static,
{
    /// Creates a cheapest-insertion phase over an explicitly keyed source.
...
impl<S, E> ListSourceAccess<S> for ListCheapestInsertionPhase<S, E>
where
    S: PlanningSolution,
    E: Copy + Send + Sync + 'static,
{
...
impl<S, E> ScoredListConstructionAccess<S> for ListCheapestInsertionPhase<S, E>
where
    S: PlanningSolution,
    E: Copy + Send + Sync + 'static,
{
    ...
    BestCb: ProgressCallback<S>,
{
    fn solve(&mut self, solver_scope: &mut SolverScope<'_, S, D, BestCb>) {
        let binding = bind_runtime_list_source(self, solver_scope.working_solution()).unwrap_or_else(
            |error| {
                panic!(
                    "ListCheapestInsertionPhase source binding failed before phase execution: {error:?}"
                )
            },
        );
