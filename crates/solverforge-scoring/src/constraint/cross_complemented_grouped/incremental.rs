    ...
    Sc: Score,
{
    ...
    pub(super) fn insert_complement(&mut self, entities_t: &[T], t_idx: usize) -> Sc {
        if t_idx >= entities_t.len() {
            return Sc::zero();
        }
        let key = (self.key_t)(&entities_t[t_idx]);
    ...
    Sc: Score,
{
    ...
    fn match_count(&self, solution: &S) -> usize {
        self.extractor_t.extract(solution).len()
    }
    ...
    fn initialize(&mut self, solution: &S) -> Sc {
        ...
        let entities_b = self.extractor_b.extract(solution);
        let entities_t = self.extractor_t.extract(solution);
        self.build_join_indexes(entities_a, entities_b);

        let mut total = Sc::zero();
        for t_idx in 0..entities_t.len() {
            total = total + self.insert_complement(entities_t, t_idx);
        }
    ...
    fn on_insert(&mut self, solution: &S, entity_index: usize, descriptor_index: usize) -> Sc {
        ...
        if t_changed {
            total = total + self.insert_complement(entities_t, entity_index);
        }
        total
    }
