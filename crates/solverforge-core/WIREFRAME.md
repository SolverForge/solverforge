...
| Method | Signature | Note |
|--------|-----------|------|
| `is_pinned` | `fn is_pinned(&self) -> bool` | Default: `false`; a manually assembled descriptor also needs `with_pin_predicate()` to expose this value to solver phases |
...
Query methods: `genuine_variable_descriptors()`, `shadow_variable_descriptors()`, `find_variable(&str)`, `has_genuine_variables()`, `has_extractor()`, `has_pin_predicate()`, `entity_count(&dyn Any)`, `is_pinned(&dyn Any, usize)`, `get_entity()`, `get_entity_mut()`, `entity_refs()`, `for_each_entity()`, `for_each_entity_mut()`
