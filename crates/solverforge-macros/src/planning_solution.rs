...
pub fn expand_derive(input: DeriveInput) -> Result<TokenStream, Error> {
    ...
    // Generate entity_count method arms
    let entity_count_arms: Vec<_> = fields
        ...
        .map(|(idx, f)| {
            let field_name = f.ident.as_ref().unwrap();
            quote! { #idx => self.#field_name.len(), }
        })
        .collect();

    // Generate list operations if configured
    let list_operations = generate_list_operations(&shadow_config);
...
/// Generates list operation methods from shadow_variable_updates configuration.
fn generate_list_operations(config: &ShadowConfig) -> TokenStream {
    let (list_owner, list_field, element_type) = match (
        &config.list_owner,
        &config.list_field,
        &config.element_type,
    ) {
        (Some(lo), Some(lf), Some(et)) => (lo, lf, et),
