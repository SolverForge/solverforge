//! Public/static/dynamic trace parity for canonical cheapest insertion.
...
use crate::runtime::compiler::executor::list_construction::execute_runtime_list_cheapest_insertion;
use crate::stats::{
...
fn runtime_descriptor() -> SolutionDescriptor {
    SolutionDescriptor::new("CheapestPlan", TypeId::of::<Plan>()).with_entity(
...
fn trace_header() -> CandidateTraceHeader {
    CandidateTraceHeader::new(
        "compiled-cheapest-parity".to_string(),
...
fn run_runtime(slot: Slot) -> (Vec<Vec<usize>>, CandidateTraceTelemetry) {
    ...
    .expect("compiled cheapest insertion executes");
    finish(scope)
}

#[test]
fn public_static_and_dynamic_cheapest_share_routes_and_candidate_trace() {
    ...
    assert!(dynamic_trace.pulls.iter().any(|pull| matches!(
        pull.source,
        crate::stats::CandidateTraceSource::ListCheapestInsertionTrial
    )));
}
