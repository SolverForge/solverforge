...
macro_rules! impl_incremental_tri_constraint {
    ($struct_name:ident) => {
            ...
            Sc: Score,
        {
            ...
            #[inline]
            fn compute_score(&self, solution: &S, a_idx: usize, b_idx: usize, c_idx: usize) -> Sc {
                let base = (self.weight)(solution, a_idx, b_idx, c_idx);
                ...
                if let Some(others) = key_to_indices.get(&key) {
                    for &i in others {
                        ...
                        for &j in others {
                            ...
                            if matches.contains(&triple) { continue; }
                            ...
                            if filter(solution, a, b, c) && matches.insert(triple) {
                                ...
                                let base = weight(solution, a_idx, b_idx, c_idx);
            ...
            Sc: Score,
        {
                        ...
                        for pos_j in (pos_i + 1)..indices.len() {
                            for pos_k in (pos_j + 1)..indices.len() {
                                ...
                                if (self.filter)(solution, a, b, c) {
                                    total = total + self.compute_score(solution, i, j, k);
                                }
                            }
                        }
                    }
            ...
            fn name(&self) -> &str { &self.constraint_ref.name }
            fn is_hard(&self) -> bool { self.is_hard }
            fn constraint_ref(&self) -> ConstraintRef { self.constraint_ref.clone() }
        ...
        impl<S, A, K, E, KE, F, W, Sc: Score> std::fmt::Debug for $struct_name<S, A, K, E, KE, F, W, Sc> {
...
macro_rules! impl_incremental_quad_constraint {
    ($struct_name:ident) => {
            ...
            Sc: Score,
        {
            ...
            #[inline]
            fn compute_score(&self, solution: &S, a_idx: usize, b_idx: usize, c_idx: usize, d_idx: usize) -> Sc {
                    ...
                    for &i in others {
                        ...
                        for &j in others {
                            ...
                            for &k in others {
                                ...
                                if matches.contains(&quad) { continue; }
            ...
            fn retract_entity(&mut self, solution: &S, entities: &[A], index: usize) -> Sc {
                if let Some(key) = self.index_to_key.remove(&index) {
                    if let Some(indices) = self.key_to_indices.get_mut(&key) {
                        indices.remove(&index);
                        if indices.is_empty() { self.key_to_indices.remove(&key); }
                    }
                }
                    ...
                    for &other in &[i, j, k, l] {
                        if other != index {
                            if let Some(other_set) = self.entity_to_matches.get_mut(&other) {
                                ...
                                if other_set.is_empty() { self.entity_to_matches.remove(&other); }
                            }
                        }
                    }
                        ...
                        for pos_j in (pos_i + 1)..indices.len() {
                            for pos_k in (pos_j + 1)..indices.len() {
                                for pos_l in (pos_k + 1)..indices.len() {
                                    ...
                                    let k = indices[pos_k];
                                    let l = indices[pos_l];
                                    if (self.filter)(solution, &entities[i], &entities[j], &entities[k], &entities[l]) {
                                        count += 1;
                                    }
                                }
                            }
...
macro_rules! impl_incremental_penta_constraint {
    ($struct_name:ident) => {
            ...
            Sc: Score,
        {
            ...
            #[inline]
            fn compute_score(&self, solution: &S, a_idx: usize, b_idx: usize, c_idx: usize, d_idx: usize, e_idx: usize) -> Sc {
                        ...
                        for &j in others {
                            ...
                            for &k in others {
                                ...
                                for &l in others {
                                    ...
                                    if matches.contains(&penta) { continue; }
                                    ...
                                    if filter(solution, a, b, c, d, e) && matches.insert(penta) {
                                        ...
                                        let base = weight(solution, a_idx, b_idx, c_idx, d_idx, e_idx);
                                        let score = match impact_type {
                                            ImpactType::Penalty => -base,
                                            ImpactType::Reward => base,
                                        };
            ...
            fn retract_entity(&mut self, solution: &S, entities: &[A], index: usize) -> Sc {
                ...
                for penta in pentas {
                    ...
                    if i < entities.len() && j < entities.len() && k < entities.len() && l < entities.len() && m < entities.len() {
                                ...
                                for pos_l in (pos_k + 1)..indices.len() {
                                    for pos_m in (pos_l + 1)..indices.len() {
                                        ...
                                        if (self.filter)(solution, a, b, c, d, e) {
                                            total = total + self.compute_score(solution, i, j, k, l, m);
                                        }
                                    }
                                }
                            }
                            ...
                            for pos_k in (pos_j + 1)..indices.len() {
                                for pos_l in (pos_k + 1)..indices.len() {
                                    for pos_m in (pos_l + 1)..indices.len() {
                                        ...
                                        let l = indices[pos_l];
                                        let m = indices[pos_m];
                                        if (self.filter)(solution, &entities[i], &entities[j], &entities[k], &entities[l], &entities[m]) {
                                            count += 1;
                                        }
                                    }
                                }
