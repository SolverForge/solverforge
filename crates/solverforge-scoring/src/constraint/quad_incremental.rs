    ...
    Sc: Score,
{
    ...
    weight: W,
    is_hard: bool,
    /// entity_index -> set of (i, j, k, l) quadruples involving this entity
    entity_to_matches: HashMap<usize, HashSet<(usize, usize, usize, usize)>>,
    /// All matched quadruples (i, j, k, l) where i < j < k < l
    ...
    Sc: Score,
{
    ...
    fn insert_entity(&mut self, solution: &S, entities: &[A], index: usize) -> Sc {
        ...
        // Find matches with all triples of other entities having the same key (zero allocation)
        let mut total = Sc::zero();
        if let Some(others) = key_to_indices.get(&key) {
            // Iterate over all triples (i, j, k) where i < j < k, excluding current index
            for &i in others {
                if i == index {
                    continue;
                }
                for &j in others {
                    ...
                    for &k in others {
                        ...
                        // Determine canonical ordering for this quadruple
                        let mut indices = [index, i, j, k];
                        indices.sort();
                        let [a_idx, b_idx, c_idx, d_idx] = indices;
    ...
    Sc: Score,
{
    fn evaluate(&self, solution: &S) -> Sc {
        ...
        // Evaluate quadruples within each key group
    ...
    fn get_matches(&self, solution: &S) -> Vec<DetailedConstraintMatch<Sc>> {
        impl_get_matches_nary!(quad: self, solution)
    }
}
    ...
    for IncrementalQuadConstraint<S, A, K, E, KE, F, W, Sc>
{
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("IncrementalQuadConstraint")
