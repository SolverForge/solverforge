...
fn scan_dynamic_scalar_best_fit<S, V, D, ProgressCb>(
    variable_index: usize,
    ...
    ProgressCb: ProgressCallback<S>,
{
    ...
    for entity_index in 0..entity_count {
        ...
        for (value_index, value) in values.into_iter().enumerate() {
            ...
            let score =
                evaluate_dynamic_scalar_assignment(phase_scope, &ctx, entity_index, Some(value));
            tracker.consider(value_index, score);
            let should_replace = match best {
                None => true,
                Some((_, _, best_score)) => score > best_score,
...
fn scan_dynamic_list_best_fit<S, V, D, ProgressCb>(
    list_index: usize,
    ...
    ProgressCb: ProgressCallback<S>,
{
    ...
    for element_index in 0..element_count {
        ...
        let mut best_for_element: Option<(usize, usize, S::Score)> = None;
        for entity_index in 0..entity_count {
            let len = ctx.list_len(phase_scope.score_director().working_solution(), entity_index);
            for position in 0..=len {
                let score =
                    evaluate_dynamic_list_insertion(phase_scope, &ctx, element, entity_index, position);
                let should_replace = match best_for_element {
                    None => true,
                    Some((_, _, best_score)) => score > best_score,
                };
